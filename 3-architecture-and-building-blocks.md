...
![](./images/13.png)
<p align=center><b>图 13</b> 存在引入延迟的效果器的链中的自动化处理组件。两个放大器的效果会相互抵消，然而应用自动化时需要考虑延迟。</p>
...
![](./images/14.png)
<p align=center><b>图 14</b> USB Type A 和 Type B 端口。端口在物理上实行结构化的连接方式。</p>

端口具有属性，其在注册（创建）端口时设置：端口可以处理的数据类型（音频或 MIDI）、端口的方向（输入或输出）以及表 2 中列入的其他标志位。

<table align=center>
<tbody>
<tr><th>是输入</th><th>端口可以接收数据（信号源，可写）</th></tr>
<tr><th>是输出</th><th>可从端口读取数据（信号槽，可读）</th></tr>
<tr><th>是终端</th><th>端口不能连接到其他的端口。<br/>例如，接收 MIDI 信号，但不传出此数据的合成器；生成不源自其他端口的数据的音频发生器（generator）。</th></tr>
<tr><th>是物理端口</th><th>端口代表实际的硬件 I/O（这种端口通常也是终端）</th></tr>
</tbody>
</table>
<p align=center><b>表 2</b> 端口的标志位</p>
...
![](./images/15.png)
<p align=center><b>图 15</b> 端口方向：从输出端口读取数据，经处理后写入输入端口，供其他端口处理。输入到输出的连接是有向的。</p>
...
作为最后的基本组件，需要考虑到辅助发送。如第 7 节中描述，链路是自动进行处理的整体对象，可以描述为带有输入和输出的黑盒。

然而，在处理过程中间将信号分出也有合理的使用场景。辅助发送至混响总线便是一种常见的用例。监听链路的处理链中特定点的信号的“监听”控制也属于这一类，侧链同样如此。

这些发送的特点在于，它们会将处理链中间的信号分出（即“发送”），并用于另一链路的处理链（即“返回”或“返送”）之中，见图 16。

辅助发送并非作为端口展现，而是完全由 Ardour 内部访问。

将当前数据发至输出端口，从输入端口读回数据的端口插入（见图 12）存在（至少）一个循环的延迟。提供这种插入是用于将数据发送到外部的信号处理组件，也只能在下一次循环将数据返回。辅助发送没有这种限制。

![](./images/16.png)
<p align=center><b>图 16</b> 带有送入总线的辅助输出的轨道。</p>

## 13 输入/输出设备和传送设备

端口（见第 10 节）提供了一套非常底层的接口，表示独立的单个声道。因此有理由建立一套更为复杂的端口表示法，从而提供高层功能以及允许为端口编组的功能。在 Ardour 中，I/O 对象负责这一用途，可以用两个单声道端口创建立体声端口。端口可进行任意方式的组合 ，包括音频和 MIDI 数据类型的混合。

I/O 对象*包含*端口，提供通用的对端口集操作的方法：连接所有端口、将所有端口的缓冲区作清除或静音处理、以及设置端口延迟。I/O 对象也可以标记为不活动状态，以跳过实际的输入和输出过程。I/O 对象的端口数量是动态的，可以在运行时添加或移除 I/O 中的端口。

I/O 对象的抽象层级还是很低，为了在 Ardour 内部使用时达成一致，要进一步作抽象。**I/O 处理组件**（IOProcessor）便负责这一点：它会使用默认的处理组件 API（类似任意插件），并将处理组件的 I/O 针脚映射到后端的端口。I/O 处理组件拥有一个或两个 I/O 元素。组件或是只有输入，或是只有输出，或是双向兼具（发送/返回）。组件的配置是在实例化时进行的。

针对特别用途，I/O 处理组件还作了进一步的扩展。最典型的便是传送设备（Delivery），其包含平衡或声像控制。

- 传送设备*是* I/O 处理组件，*是*处理组件
- I/O 处理组件*包含*端口
- 传送设备*包含*声像器

用处理组件 API 对 I/O 作抽象有着明显的优势，即控制其参数的处理方式可与其他的处理组件一致。

图 17 展示了其他从 I/O 处理组件派生的类。最重要的是之前提到的发送组件。其*包含*可对信号作延迟处理。再将其写入端口的延迟处理器。

![](./images/17.png)
<p align=center><b>图 17</b> I/O 处理设备类的继承图。（由 Doxygen 生成，Ardour 6.0-pre0-53-g94cce9e06）</p>

由此，可以改良第 7 节中的定义：将“链路*包含* I/O 端口”更精确地描述：
- 对于输入：链路*包含* I/O，I/O *包含*端口
- 对于输出：链路*包含*传送设备，传送设备是 I/O 处理组件，I/O 处理组件*包含*端口